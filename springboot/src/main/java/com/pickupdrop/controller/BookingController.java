...
public class BookingController {
    ...
    /** Joinable groups for this trip's landing week (owner only, no personal data). */
    @GetMapping("/{bookingId}/group-suggestions")
    public TravelGroupDto.SuggestionsResponse suggestGroups(@PathVariable String bookingId) {
        return bookingFacade.suggestGroups(
                AuthorizationUtil.getCurrentUser().getUserId(), bookingId);
    }

    /** Pick a group for this trip; null groupId starts a new one for the week. */
    @PutMapping("/{bookingId}/group")
    public BookingDto.Response selectGroup(
            @PathVariable String bookingId,
            @RequestBody BookingDto.SelectGroupRequest request) {
        return bookingFacade.selectGroup(
